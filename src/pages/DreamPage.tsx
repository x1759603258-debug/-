...
import { AttachmentInput, ChatMessage as ChatMessageType, ChatSession, MessageFile, gardenApi } from "../lib/desktopApi";
import { isDreamDraftContent } from "../lib/dreamIntent";
...
interface DreamPageProps {
  actions: AppActions;
  onOpenHuaying: (draft: { prompt: string; files: AttachmentInput[] }) => void;
}

export function DreamPage({ actions, onOpenHuaying }: DreamPageProps) {
  ...
  useEffect(() => {
    gardenApi.listDreams().then((items) => {
  ...
  useEffect(() => {
    return gardenApi.onDreamChunk((chunk) => {
  ...
  const confirmableMessage = [...(active?.messages ?? [])].reverse().find((message) => (
    (message.confirmImage || (message.role === "assistant" && isDreamDraftContent(message.content)))
    && !message.imageUrl
  ...
  async function newDream() {
    const session = await gardenApi.createDream();
  ...
  async function confirmDelete() {
    ...
    await gardenApi.deleteDream(deleteTarget.id);
  ...
  async function send(message = draft.trim(), files = draftFiles, sessionId = active?.id, retryMessageId?: string) {
    ...
    const baseSession = active ?? createFallbackSession("新的绘梦");
    ...
    setLoading(true);
    try {
      const savedSession = await gardenApi.sendDreamMessageStream(request);
  ...
  function stop() {
    if (!pending) return;
    sendTokenRef.current += 1;
    void gardenApi.stopDream(pending.sessionId);
  ...
  function fileForTransfer(file: MessageFile): AttachmentInput | undefined {
    if (!file.dataUrl || !file.mimeType.startsWith("image/")) return undefined;
    return {
      name: file.name,
  ...
  function filesForDraft(draftMessage: ChatMessageType) {
    if (!active) return [];
    const draftIndex = active.messages.findIndex((message) => message.id === draftMessage.id);
    const beforeDraft = draftIndex >= 0 ? active.messages.slice(0, draftIndex) : active.messages;
    return ([...beforeDraft].reverse().find((message) => message.role === "user" && message.files?.some((file) => file.dataUrl))?.files ?? [])
      .map(fileForTransfer)
      .filter((file): file is AttachmentInput => Boolean(file));
  }

  function confirmImageDraft() {
    if (!active || !confirmableMessage) return;
    onOpenHuaying({
      prompt: confirmableMessage.content,
      files: filesForDraft(confirmableMessage)
    });
  }

  return (
    <section className="page dream-page fade-in">
      <header className="page-heading compact-heading">
        <div>
          <span className="eyebrow">绘梦</span>
          <h2>想画什么，我们先聊清楚。</h2>
        </div>
        <div className="heading-side">
          <p>温柔帮你聊清构思、分析参考图、整理小稿；真正出图请去花映完成。</p>
      ...
      <div className="workspace-layout">
        <aside className="session-list">
          <div className="session-head">
            <strong>绘梦</strong>
            <button className="session-add" onClick={newDream}><Plus size={17} /></button>
          </div>
          {sessions.length === 0 && <p className="muted">还没有绘梦。</p>}
        ...
        <section className="chat-card">
          <div className="chat-title"><strong>{active?.title ?? "新的绘梦"}</strong><span>构思引导 · 小稿整理</span></div>
          <div className="message-scroll" ref={scrollRef}>
            {!active?.messages.length && <div className="empty-state">先描述你想看见的画面。</div>}
          ...
          <Composer
            ...
            value={draft}
            files={draftFiles}
            placeholder="描述画面、拖入参考图，或说“帮我整理小稿”。"
            confirmLabel={confirmableMessage ? "去花映生成" : undefined}
            onConfirm={confirmImageDraft}
            onChange={setDraft}
      ...
      {deleteTarget && (
        <div className="confirm-backdrop" onClick={() => setDeleteTarget(undefined)}>
          <div className="confirm-card" onClick={(event) => event.stopPropagation()}>
            <strong>删除这段绘梦？</strong>
            <p>删除后，这个绘梦文件夹里的消息和参考文件都会移除。</p>
